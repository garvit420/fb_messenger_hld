...
class TestGetConversationMessages:
    ...
    def test_get_messages_success(
        ...
        response = client.get(
            f"/api/messages/conversation/{test_conversation.id}",
            headers=auth_headers
        )
    ...
    def test_get_messages_not_participant(
        ...
        response = client.get(
            f"/api/messages/conversation/{other_conv.id}",
            headers=auth_headers
        )
...
class TestDeleteMessage:
    ...
    def test_delete_own_message(
        self, client: TestClient, auth_headers, test_message
    ):
    ...
    def test_delete_others_message(
        self, client: TestClient, auth_headers2, test_message
    ):
...
class TestUpdateMessageStatus:
    ...
    def test_update_status_to_read(
        self, client: TestClient, auth_headers2, test_message
    ):
...
class TestMarkConversationAsRead:
    ...
    def test_mark_as_read_success(
        ...
        response = client.put(
            f"/api/messages/conversation/{test_conversation.id}/read",
            headers=auth_headers2
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_mark_as_read_not_participant(
        self, client: TestClient, auth_headers, db_session
    ):
        ...
        response = client.put(
            f"/api/messages/conversation/{other_conv.id}/read",
            headers=auth_headers
        )
        assert response.status_code == 403
