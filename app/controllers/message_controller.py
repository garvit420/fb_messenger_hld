...
from app.schemas.message import (
    ...
    PaginatedMessageResponse, MessageStatus
)
...
class MessageController:
    ...
    def _find_or_create_conversation(
        ...
        user1_convos = db.query(ConversationParticipant.conversation_id).filter(
            ConversationParticipant.user_id == user1_id
        ).subquery()

        # Find conversation where user2 is also a participant
        existing_convo = db.query(ConversationParticipant.conversation_id).filter(
            and_(
        ...
        if existing_convo:
            return db.query(Conversation).filter(
                Conversation.id == existing_convo[0]
            ).first()
    ...
    async def get_messages_before_timestamp(
        self, db: Session, user: User, conversation_id: str,
        before_timestamp: datetime, page: int = 1, limit: int = 20
    ) -> PaginatedMessageResponse:
    ...
    async def delete_message(
        self, db: Session, user: User, message_id: str
    ) -> MessageResponse:
